---
title: "Exploratory analysis and explanation of GenomeRunner results"
...
* Several R packages, shown below

Example
---
* Disease- and trait-specific sets of SNPs from GWAScatalog (FOIs) will be analyzed for the enrichment in 161 transcription factor binding sites (TFBSs, GFs) using GenomeRunner web server;
...
fn_degs <- "results/clusters-degs_all.txt" # Differential GFs among the clusters
```
...
===
Regulatory similarity analysis groups SNP sets by correlation of their regulatory enrichemt profiles, or sets of the transformed p-values. The NxN square matrix of correlation coefficients is assembled, where N is the number of SNP sets.
...
Most regulatory correlated sets of SNPs
===
...
===
The heatmap object contains information about the clustering. This information can be visualized as a dendrogram, cut into separate clusters defined by the cut height (user defined). The cluster ordering is outputted into the `r print(fn_clust)` file. 
...
The p-values are -log10-transformed. The distributions of these transformed p-values can be tested for statistically significant differences among the clusters using the Bioconductor *limma* package.

First, the cluster groups and labels are defined. If the number of FOI sets in each cluster is less than the minimum number (user defined), such cluster is not considered for differential enrichment.
...
For the limma analysis, the ExpressionSet, the design matrix, the thresholds are defined. The limma analysis is performed for each pair of clusters. The results of limma analysis are outputted into the `r print(fn_degs)` file.
...
===
Clstering and visualization of the enrichment results is the main heatmap generated by GenomeRunner. It is the fastest way to overview which regulatory datasets enriched where, and how strong. Due to large number of regulatory datasets typically used for the analysis, GenomeRunner filters those that do not show enrichment in any of the sets of SNPs. This tutorial will show how to tweak the default filtering parameters to visualize the most significant enrichments.

First, the minimum number of times an regulatory element should show statistically significant associations should be defined - at least once in this tutorial. Then, the distributions of the transformed p-value and SD are examined, and the corresponding cutoffs for matrix filtering are set. The higher the p-value cutoff - the more regulatory datasets will be filtered. The SD cutoff is used to filter out similarly enriched regulatory datasets and bring up the most differentially enriched regulatory datasets.
...
Both rows and columns are filtered. It is possible to either take the top 10 regulatory datasets most differentially enriched across the sets of SNPs, or to filter by the p-value cutoff.
