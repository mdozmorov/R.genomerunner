...
Analysis of regulatory elements differentially associated with the tumor-associated SNPs in the two groups identified a set of transcription factors.

The "Enrichment 1/2" columns show the average p-values of the group-specific SNPs-regulatory associations. A "-" sign indicates that an association is underrepresented. E.g., the TRIM28 binding sites are significantly enriched in the tumor-associated SNPs from the first group, and are non-significantly depleted in the SNPs from the second group.
...
```{r limmaOnClusters, echo=FALSE, warning=FALSE}
for (i in 1:length(c$lower)) { # Cycle through each cluster combinations
  for (j in 1:length(c$lower)) {
    if (i < j) { # Avoid testing the same pairs, e.g., do not repeat 1vs2 2vs1 tests
      ...
      d <- apply(mtx, 1, function(x) {t.test(x[eset.groups == i], x[eset.groups == j])$p.value})
    }
  }
}

degs <- d[d < 0.05] # Keep only significant differences
...
degs.pvals.log <- cbind(rowMeans(mtx[rownames(degs), eset.groups == 1]), rowMeans(mtx[rownames(degs), eset.groups == 2]))
...
degs.table <- merge(degs, trackDb.hg19, by="row.names") # Merge with the descriptions
colnames(degs.table) <- c("Table name", "p-value", "Enrichment 1", "Enrichment 2", "Description")
pander(degs.table[, c(5, 3, 4, 2)])
```

**Text mining question 2:** Are the names of the transcription factors, TIM28, GATA3 and TCF12, associated stronger with the tumors in the first cluster based on the literature strength? Are they associated (statistically significantly) weaker with the second cluster? Are the transcription factors themselves related based on the literature? *Expected answer:* Yes, the literature associations should confirm the relationships.
...
The regulatory and co-morbidity-based (Phi-correlations) disease-disease correlations correlate with each other at Pearson's correlation coefficient of 0.54 (p-value = 0). Using "relative risk" co-morbidity correlations produces similar results.

**Text mining question 3:** If we take all 21 tumor types, map them to SNOMED IDs and get pair-wise correlations among the SNOMED IDs based on literature strength (or any other literature-based metrics) - what are the literature-based and regulatory correlations? *Expected answer:* We have already seen that it is correlated very well, even better than co-morbidities. We need to put it into a reproducible piece of code to share along with the publication.
