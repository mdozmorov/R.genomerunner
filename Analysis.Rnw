...
The enrichment analysis results are outputted in the \texttt{matrix.txt} file, stored as the enrichment p-values with a "-" sign added to denote depletion. We combine the results of different analyses into one matrix and transform the raw p-values using -log10 transformation, and keep the "-" sign for the case of depletion.
<<loadData, echo=TRUE>>=
# Define output and data subfolders to use, change to analyze different data
rname<-"data//results//" # Output folder
# List of GenomeRunner Web results data folders
dname<-c("data//more15_vs_tfbsEncode//", "data//more15_vs_Other//")
...
abline(h=8) # Visually evaluate the height where to cut
c<-cut(h$colDendrogram, h=8) 
...
First, we define the cluster groups and labels. Clusters that contain less than a minimum number of elements are not considered, as we cannot test statistically significant differences with them.
