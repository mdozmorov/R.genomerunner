...
rname<-"data//Raychaudhuri_results//" # Output folder
# One or more GenomeRunner Web results data folders.
dname<-c("data//Raychaudhuri_vs_tfbsEncode/", "data//Raychaudhuri_vs_Selected//", "data//Raychaudhuri_vs_ENCODE//", "data//Raychaudhuri_vs_Other//")
...
par(oma=c(1, 0, 0, 0), mar=c(20, 4.1, 4.1,2.1), cex=0.8)
...
abline(h=2) # Visually evaluate the height where to cut
c<-cut(h$colDendrogram, h=2) 
...
cutoff.p<- -log10(0.0001); cutoff.sd<-0.6
@
...
par(oma=c(10, 0, 0, 10), mar=c(5.1, 4.1, 4.1,2.1), cex=0.5)
